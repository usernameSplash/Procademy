...
bool INetServer::Terminate(void)
{
...
{
	Session* session = TrySessionAccess(sessionId);
...
unsigned int WINAPI INetServer::AcceptProc(void* arg)
{
	...
	while (true)
	{
		...
		instance->IncrementSessionUseCount(newSession);
		...
		instance->DecrementSessionUseCount(newSession);
	}
...
unsigned int WINAPI INetServer::IOCPWorkerProc(void* arg)
{
	...
	while (true)
	{
		...
		if (instance->TrySessionAccess(session->_id) == nullptr)
		{
			continue;
		}
		...
		if (gqcsRet == 0 || bytes == 0)
		{
			...
			{
				int errorCode = WSAGetLastError();
				if (errorCode != WSAEINTR && errorCode != WSAECONNABORTED && errorCode != WSAECONNRESET && errorCode != WSAENOTSOCK)
				{
		...
		{
			instance->RecvHandler(session, bytes);
		}
		...
		instance->DecrementSessionUseCount(session); // To Decrement Use Count Increased By Recv/Send Post;
		instance->ReturnSessionAccess(session); // To Release Session Access in Worker Thread;
	}
	return 0;
}
...
bool INetServer::RecvPost(Session* session)
{
	...
	IncrementSessionUseCount(session);
	...
	if (recvRet == SOCKET_ERROR)
	{
		...
		if (errorCode != WSA_IO_PENDING)
		{
			...
			DecrementSessionUseCount(session);
		}
		...
		{
			wprintf(L"# Unhandled RecvPost Error : %d\n", errorCode);
		}
...
bool INetServer::SendPost(Session* session)
{
	...
	if (sendRet == SOCKET_ERROR)
	{
		...
		{
			wprintf(L"# Unhandled SendPost Error : %d\n", errorCode);
		}
...
Session* INetServer::TrySessionAccess(const SessionID sessionId)
{
...
void INetServer::ReturnSessionAccess(Session* session)
{
	DecrementSessionUseCount(session);
}

void INetServer::IncrementSessionUseCount(Session* session)
{
	InterlockedIncrement16(&session->_flag._useCount);
}

void INetServer::DecrementSessionUseCount(Session* session)
{
	...
	if (useCount == 0)
	{
		...
		{
			PostQueuedCompletionStatus(_iocp, 1, session->_id, (LPOVERLAPPED)&session->_releaseOvl);
		}
	}
