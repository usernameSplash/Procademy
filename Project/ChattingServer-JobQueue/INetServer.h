...
private:
	bool RecvHandler(Session* session, const int byte);
	bool SendHandler(Session* session, const int byte);
	void ReleaseHandler(Session* session);

	// Session List Manager Function
private:
	Session* TrySessionAccess(const SessionID sessionId, int line);
	void ReturnSessionAccess(Session* session, int line);
	void IncrementSessionUseCount(Session* session, int line);
	void DecrementSessionUseCount(Session* session, int line);
