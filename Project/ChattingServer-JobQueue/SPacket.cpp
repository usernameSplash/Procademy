...
ObjectPool<SPacket> SPacket::_packetPool = ObjectPool<SPacket>(5000);
...
bool SPacket::Decode(PacketHeader& header)
{
	...
	unsigned char ePrev = e;
	unsigned char pPrev = p;
	...
	{
		e = *(payloadPtr + iCnt);
		p = e ^ (ePrev + ENCODE_KEY + (iCnt + 2));
		d = p ^ (pPrev + header._randKey + (iCnt + 2));
...
{
	memcpy(mBuffer, header, mHeaderLen);
}

void SPacket::GetHeaderData(void* outHeader)
{
	memcpy(outHeader, mBuffer, mHeaderLen);
}
...
__int64 SPacket::PoolBlockSize(void)
{
	return _packetPool.Size();
}

__int64 SPacket::PoolNodeSize(void)
{
