...
1. CNetRecvPacket::Alloc으로 할당, 데이터 삽입 전 Clear로 초기화.
...
class CNetGroup;
class CNetMsg;
class CNetRecvPacket
{
	friend CTlsPool<CNetRecvPacket>;
	friend CNetMsg;

public:
	static CNetRecvPacket* Alloc();
	static bool Free(CNetRecvPacket* packet);
	...
	CLockFreeStack<CNetMsg*>* _msgs;
	static CTlsPool<CNetRecvPacket> _pool;
...
protected:
	CNetRecvPacket()
		: _iBufferSize(dfSPACKET_DEF_SIZE)
		, _iPayloadSize(0)
		, _iHeaderSize(dfNETHEADER_LEN)
		, _iPayloadReadPos(dfNETHEADER_LEN)
		, _iPayloadWritePos(dfNETHEADER_LEN)
		, _iHeaderReadPos(0)
		, _iHeaderWritePos(0)
	{
		_chpBuffer = new char[_iBufferSize];
		_msgs = new CLockFreeStack<CNetMsg*>;
	}

	CNetRecvPacket(int iBufferSize)
...
public:
	inline CNetRecvPacket& operator = (CNetRecvPacket& clSrCNetRecvPacket)
	{
		*this = clSrCNetRecvPacket;
		return *this;
	}

	inline CNetRecvPacket& operator << (float fValue)
	{
	...
	inline CNetRecvPacket& operator << (double dValue)
	{
	...
	inline CNetRecvPacket& operator << (char chValue)
	{
	...
	inline CNetRecvPacket& operator << (unsigned char byValue)
	{
	...
	inline CNetRecvPacket& operator << (short shValue)
	{
	...
	inline CNetRecvPacket& operator << (unsigned short wValue)
	{
	...
	inline CNetRecvPacket& operator << (int iValue)
	{
	...
	inline CNetRecvPacket& operator << (long lValue)
	{
	...
	inline CNetRecvPacket& operator << (__int64 iValue)
	{
		if (_iBufferSize - _iPayloadWritePos < sizeof(iValue))
			Resize((int)(_iBufferSize * 1.5f));
	...
	inline CNetRecvPacket& operator >> (BYTE& byValue)
	{
	...
	inline CNetRecvPacket& operator >> (wchar_t& szValue)
	{
	...
	inline CNetRecvPacket& operator >> (WORD& wValue)
	{
	...
	inline CNetRecvPacket& operator >> (DWORD& dwValue)
	{
...
public:
	inline void CopyRecvBuf(CNetRecvPacket* origin)
	{
