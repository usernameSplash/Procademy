...
bool CChattingServer::Initialize()
{
	...
	for (int i = 0; i < dfSECTOR_CNT_Y; i++)
	{
		for (int j = 0; j < dfSECTOR_CNT_X; j++)
		{
			...
			if (i > 0)
			{
				_sectors[i][j]._around.push_back(&_sectors[i - 1][j]);
			}

			if (i > 0 && j > 0)
			{
				_sectors[i][j]._around.push_back(&_sectors[i - 1][j - 1]);
			}

			if (i > 0 && j < dfSECTOR_CNT_Y - 1)
			{
				_sectors[i][j]._around.push_back(&_sectors[i - 1][j + 1]);
			}

			if (i < dfSECTOR_CNT_X - 1)
			{
				_sectors[i][j]._around.push_back(&_sectors[i + 1][j]);
			}

			if (i < dfSECTOR_CNT_X - 1 && j > 0)
			{
				_sectors[i][j]._around.push_back(&_sectors[i + 1][j - 1]);
			}

			if (i < dfSECTOR_CNT_X - 1 && j < dfSECTOR_CNT_Y - 1)
			{
				_sectors[i][j]._around.push_back(&_sectors[i + 1][j + 1]);
			}
			...
			if (j > 0)
			{
				_sectors[i][j]._around.push_back(&_sectors[i][j - 1]);
			}

			if (j < dfSECTOR_CNT_Y - 1)
			{
				_sectors[i][j]._around.push_back(&_sectors[i][j + 1]);
			}
		}
	...
	wprintf(L"IOCP Thread Number : ");
	scanf_s("%d", &totalThreadCnt);
	wprintf(L"Concurrent IOCP Thread Number : ");
...
unsigned int WINAPI CChattingServer::UpdateThread(void* arg)
{
...
unsigned int WINAPI CChattingServer::TimeoutThread(void* arg)
{
	CChattingServer* pServer = (CChattingServer*)arg;
